...
def confirm_output(data, torch_model, onnx_sess, trt_engine, trt_ctx):
    """确认不同模型输出是否一致"""
    ...
    print('torch_output: {}'.format(torch_output.cpu().detach().numpy()))
    ...
    print('onnx_output: {}'.format(onnx_output[0]))
    print('diff: {}'.format(torch_output.cpu().detach().numpy() - onnx_output[0]))
